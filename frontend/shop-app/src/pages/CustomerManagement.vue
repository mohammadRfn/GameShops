  ...
  <div class="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-amber-900">
    ...
    <aside :class="[
      ...
      <div class="p-8 border-b border-amber-400/30">
        <div class="flex items-center justify-between mb-6">
          <div class="flex items-center space-x-3">
            <div
              class="w-12 h-12 bg-gradient-to-r from-amber-400 to-yellow-500 rounded-2xl shadow-lg flex items-center justify-center">
      ...
      <nav class="p-4 space-y-2 overflow-y-auto h-[calc(100vh-200px)]">
        <router-link v-for="(item, index) in menuItems" :key="index" :to="item.path" @click="toggleSideMenu"
          ...
          <span
            class="w-12 h-12 bg-gradient-to-r from-amber-400/30 to-purple-500/30 rounded-xl flex items-center justify-center shadow-md group-hover:scale-110 transition-transform duration-200 flex-shrink-0">
    ...
    <main class="w-full min-h-screen transition-all duration-300" :class="{ 'mr-80': sideMenuOpen }">
      <!-- Header Section -->
      <header
        class="bg-gradient-to-r from-slate-900/95 via-purple-900/80 to-amber-900/90 backdrop-blur-xl border-b-2 border-amber-400/30 shadow-2xl p-8 sticky top-0 z-30">
        <div class="flex items-center justify-between">
          <div>
            <h1
              class="text-4xl md:text-5xl font-black bg-gradient-to-r from-amber-400 via-yellow-400 to-orange-500 bg-clip-text text-transparent drop-shadow-2xl">
              🎮 Game Empire
            </h1>
      ...
      <div class="container mx-auto p-8 lg:p-12 text-white">
        ...
        <div class="bg-slate-800/50 backdrop-blur-xl rounded-2xl p-6 mb-8 border border-amber-400/30">
          <div class="flex flex-col lg:flex-row gap-4">
            <input v-model="searchQuery" @input="debouncedSearch" type="text" placeholder="🔍 جستجو در نام، ایمیل..."
        ...
        <!-- Customers Table -->
        <div
          class="bg-gradient-to-r from-slate-900/90 via-purple-900/70 to-amber-900/90 backdrop-blur-xl rounded-3xl p-8 border border-amber-400/30 shadow-2xl">
          <!-- Loading -->
          <div v-if="loading" class="text-center py-12">
            <div class="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-amber-400 mx-auto mb-4">
            </div>
